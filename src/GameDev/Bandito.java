...
public class Bandito extends Monster {
    ...
    public Bandito(String name_, int level_)
    {
        super(name_);
    ...
    private void addArmour()
    {
        ...
        if(this.intLevel > 20)
        {
            // If GameDev.Bandito level is greater than 20
            // Pick a random number between 11 and 20
            tempNum = randNum.nextInt(10) + 11;
        }
        else if(this.intLevel > 10)
        {
            // Else if GameDev.Bandito level is between 10 and 20
    ...
    {
        // Roll to determine if the bandit is to have one of the keys
    ...
    private void rollForWeapon()
    {
        ...
        if (tempNum == 0) { this.mobWeapon = new MonsterDagger(); }
        else if (tempNum == 1) { this.mobWeapon = new MonsterSword(); }
    }

    void Equip(Weapon nonexistantweapon)
    {
        // Character only method, does nothing for monster
    }
